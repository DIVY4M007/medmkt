...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!['hospital', 'pharmacy'].includes(orgType)) {
      return NextResponse.json({ error: 'orgType must be hospital or pharmacy' }, { status: 400 });
    }
    ...
    const org = await db.organization.create({
      data: { name: orgName, type: orgType, accountType: 'buyer' }
    });
